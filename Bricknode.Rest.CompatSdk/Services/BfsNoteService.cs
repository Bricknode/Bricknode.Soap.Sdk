...
namespace Bricknode.Soap.Sdk.Services
{
    ...
    public class BfsNoteService : BfsServiceBase, IBfsNoteService
    {
        ...
        public async Task<GetNotesResponse> GetNotesAsync(GetNotesArgs filters, string? bfsApiClientName = null)
        {
            ...
            var client = await GetClientAsync<RestApi.NoteClient>(bfsApiClientName);
            var restResponse = await client.SearchAsync(BfsJsonMapper.Map<RestApi.GetNotesRequest>(request));
            var response = BfsJsonMapper.Map<GetNotesResponse>(restResponse)!;
        ...
        public async Task<CreateNotesResponse> CreateNotesAsync(CreateNote[] notes, string? bfsApiClientName = null)
        {
            ...
            var client = await GetClientAsync<RestApi.NoteClient>(bfsApiClientName);
            var restResponse = await client.CreateAsync(BfsJsonMapper.Map<RestApi.CreateNotesRequest>(request));
            var response = BfsJsonMapper.Map<CreateNotesResponse>(restResponse)!;
            ...
            UpdateNoteFields fieldsToUpdate, string? bfsApiClientName = null)
        {
            ...
            var client = await GetClientAsync<RestApi.NoteClient>(bfsApiClientName);
            var restResponse = await client.UpdateAsync(BfsJsonMapper.Map<RestApi.UpdateNoteRequest>(request));
            var response = BfsJsonMapper.Map<UpdateNoteResponse>(restResponse)!;
